...
#             F = T.T*(R*sum(T,2))
    ...
    sum_FR[:,j]=v*np.exp(1j*4*np.pi*(frecs.T)/ct*z0)

sum_FR = sum_FR*c_t**2
sum_F_t = np.fft.ifft(sum_FR, axis=0)
# sum_F_t = run_main(pos,f,freqind,nodes_steps,interfase_points,kt,ka,SL2_r,c_t,zmin,t2,E,frecs,ct)
a = np.abs(sum_F_t)

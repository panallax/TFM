...
@ray.remote
def run(j, i,pos,f,freqind,nodes_steps,interfase_points,kt,ka,SL2_r,c_t,zmin,t2,E,frecs,ct):
    t1 = time.time()
    v = np.zeros((1,f))
...
#             T = mat_T(Nodes, points, k[i])
        ...
        dst = cdist(nodes_steps[:,:,j], interfase_points,"euclidean").T
        T1 = np.exp(-1j*kt[i]*dst[i])/dst
        #t2_ = time.time()
        #print("1",t2_-t1)
...
#             F = T.T*(R*sum(T,2))
        ...
        print(time.time()-t1)
    return v.T*np.exp(1j*2*np.pi*(frecs.T)/ct*zmin)


sum_FR = np.zeros((len(frecs),len(pos)))
for j in prange(len(pos)):
    t = time.time()
    for i in range(f):
      np.append(sum_FR, run.remote(j, i,pos,f,freqind,nodes_steps,interfase_points,kt,ka,SL2_r,c_t,zmin,t2,E,frecs,ct))
...
sum_F_t= run(pos,f,freqind,nodes_steps,interfase_points,kt,ka,SL2_r,c_t,zmin,t2,E,frecs,ct)
